...
Here, we present the simulation study in the Supplement. The setup is identical to the simulation study in the main text with an additional scenario: $n_{\text{event}} = 600$.
...
res_supp    <- readRDS(file = "~/Documents/UNC/Dissertation/super prior/Code/Sims_supp/Results/compiled_Sims.rds")
res_avg_wts <- rbind(res$res_avg_wts, res_supp$res_avg_wts)
res_avg_trt <- rbind(res$res_avg_trt, res_supp$res_avg_trt)
rm(res, res_supp)
...
We focus on simulation results under censoring proportion being 0.4.
...
## supp_sims_aft_weights_nevents_600
```
...
plt
## supp_sims_aft_metrics_nevents_600
```
