...
p1   <- survminer::ggsurvplot(km_fit1, 
  ...
  labs(subtitle = "E1690", color = "Treatment", fill = "Treatment", x = "time", y = "survival probability") +
...
p2   <- survminer::ggsurvplot(km_fit2, 
                      ...
                      legend.labs = c("control", "treatment"),
                      ggtheme = theme_bw())$plot + 
  labs(subtitle = "E1684", color = "Treatment", fill = "Treatment", x = "time", y = "survival probability") +
  theme(legend.position = "bottom",
        legend.text=element_text(size=10),
        legend.key.width= unit(2, 'cm'), 
        plot.subtitle = element_text(hjust = 0.5, size = 12))
...
estim_tab <- estim_all[c("Reference", "BHM", "Commensurate", "LEAP", "NPP", "PP", "BMA", "Stacking"), c("prior", "mean", "sd", "q2.5", "q97.5", "prob_greater_0")]
...
tab <- estim_tab %>%
  dplyr::select(prior, mean, sd, CI, prob_greater_0, weights_BMA, weights_stacking) %>%
  kable("latex",
      col.names = c('Method', 'Mean', "SD", '$95\\%$ CI', 'P($\\Delta > 0$)', 'BMA Weight', 'Stacking Weight'),
