...
res_avg_trt_long <- res_avg_trt %>%
  pivot_longer(
    cols = all_of(c("avg_log_var", "bias", "log_mse", "ci.cov", "avg_interval_score")),
    names_to = "metrics",
...
res_avg_trt_long$metrics <- factor(
  res_avg_trt_long$metrics, levels = c("Bias", "Log(MSE)", "Avg Log(Var)", "Avg Interval Score", "Coverage Prob"), ordered = T
)
...
# Plots of Average Construct Weights
...
## sims_aft_weights, width: 1025, height: 600
```
...
## supp_sims_aft_weights_0.6, width: 1025, height: 600
```


# Plots of Metrics on Estimated Treatment Effect
...
plt
## sims_aft_metrics, width: 1100, height: 950
```
...
plt
## supp_sims_aft_metrics_0.2, width: 1100, height: 950
```
...
plt
## supp_sims_aft_metrics_0.6, width: 1100, height: 950
```